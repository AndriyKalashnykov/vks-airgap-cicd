...
The **go** app in this repo's six-language demo. All six serve an **identical page** — that is
...
```sh
go test ./...      # tests
go build ./...     # build -> the compiled binary
./gowebapp       # run it, then open http://localhost:8080
```
...
```sh
make app-test  APP=gowebapp
make app-build APP=gowebapp
make app-run   APP=gowebapp
```
...
1. In Gitea, open **`demo/gowebapp-app`** -> **`main.go`** and bump the version
   (Go has no manifest version field, so it is a source constant):

   ```text
   const appVersion = "0.1.0"          ->   const appVersion = "0.1.1"
   ```
...
2. Commit. The push fires the webhook and the pipeline runs: clone -> test -> build -> push to
   Harbor -> write the new tag back into `gowebapp-deploy`.

3. Watch it: the Tekton dashboard, then Harbor (`apps/gowebapp` gains an artifact tagged with your new
   version **and** the commit sha, on one digest).

4. ArgoCD syncs the deploy repo and rolls the pod. Refresh **`gowebapp.vks.local`** — `Deployed tag`
   reads your new version, and `Commit` reads the sha that built it.

Each app declares its own version in its own file, so bumping this one changes **only gowebapp**.
...
| | |
|---|---|
| this source | is the content of the Gitea repo **`demo/gowebapp-app`** |
| its manifests | `deploy/gowebapp/` -> the Gitea repo **`demo/gowebapp-deploy`** (ArgoCD's source) |

...
The **python** app in this repo's six-language demo. All six serve an **identical page** — that is
...
```sh
pytest      # tests
pip install -r requirements.txt     # build -> (none — interpreted)
python app.py       # run it, then open http://localhost:8080
```
...
1. In Gitea, open **`demo/pythonwebapp-app`** -> **`app.py`** and bump the version
   (there is no pyproject.toml, so it is the module dunder):
...
2. Commit. The push fires the webhook and the pipeline runs: clone -> test -> build -> push to
   Harbor -> write the new tag back into `pythonwebapp-deploy`.

3. Watch it: the Tekton dashboard, then Harbor (`apps/pythonwebapp` gains an artifact tagged with your new
   version **and** the commit sha, on one digest).

4. ArgoCD syncs the deploy repo and rolls the pod. Refresh **`pythonwebapp.vks.local`** — `Deployed tag`
   reads your new version, and `Commit` reads the sha that built it.

Each app declares its own version in its own file, so bumping this one changes **only pythonwebapp**.
...
| | |
|---|---|
| this source | is the content of the Gitea repo **`demo/pythonwebapp-app`** |
| its manifests | `deploy/pythonwebapp/` -> the Gitea repo **`demo/pythonwebapp-deploy`** (ArgoCD's source) |

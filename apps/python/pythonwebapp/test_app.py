...
def _client(p=None):
    return new_app(p or {"app_name": "a", "message": "m", "version": "v", "commit": "c"}).test_client()
...
def test_index_renders_message_version_and_commit():
    ...
    p = {"app_name": "pythonwebapp", "message": "marker-4711-hello", "version": "1.2.3", "commit": "abc1234"}
...
def test_html_significant_characters_are_escaped():
    out = render({"app_name": "a", "message": "<script>x</script>", "version": "v", "commit": "c"})
...
def test_ui_contract_producer():
    ...
    if not out:
        return
    body = _client({"app_name": "APPNAME", "message": "MESSAGE", "version": "VERSION", "commit": "COMMIT"}) \

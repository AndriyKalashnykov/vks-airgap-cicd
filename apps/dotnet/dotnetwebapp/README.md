...
The **dotnet** app in this repo's six-language demo. All six serve an **identical page** — that is
...
dotnet build     # build -> bin/
dotnet run       # run it, then open http://localhost:8080
```
...
1. In Gitea, open **`demo/dotnetwebapp-app`** -> **`dotnetwebapp.csproj`** and bump the version
   (baked into the assembly, which is where the app reads it from):
...
2. Commit. The push fires the webhook and the pipeline runs: clone -> test -> build -> push to
   Harbor -> write the new tag back into `dotnetwebapp-deploy`.

3. Watch it: the Tekton dashboard, then Harbor (`apps/dotnetwebapp` gains an artifact tagged with your new
   version **and** the commit sha, on one digest).

4. ArgoCD syncs the deploy repo and rolls the pod. Refresh **`dotnetwebapp.vks.local`** — `Deployed tag`
   reads your new version, and `Commit` reads the sha that built it.

Each app declares its own version in its own file, so bumping this one changes **only dotnetwebapp**.
...
| | |
|---|---|
| this source | is the content of the Gitea repo **`demo/dotnetwebapp-app`** |
| its manifests | `deploy/dotnetwebapp/` -> the Gitea repo **`demo/dotnetwebapp-deploy`** (ArgoCD's source) |

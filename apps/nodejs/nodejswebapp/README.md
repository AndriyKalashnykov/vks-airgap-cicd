...
The **nodejs** app in this repo's six-language demo. All six serve an **identical page** — that is
...
```sh
npm test      # tests
npm ci     # build -> node_modules/
node server.js       # run it, then open http://localhost:8080
```
...
1. In Gitea, open **`demo/nodejswebapp-app`** -> **`package.json`** and bump the version
   (the standard npm field; the app reads it at startup):
...
2. Commit. The push fires the webhook and the pipeline runs: clone -> test -> build -> push to
   Harbor -> write the new tag back into `nodejswebapp-deploy`.

3. Watch it: the Tekton dashboard, then Harbor (`apps/nodejswebapp` gains an artifact tagged with your new
   version **and** the commit sha, on one digest).

4. ArgoCD syncs the deploy repo and rolls the pod. Refresh **`nodejswebapp.vks.local`** — `Deployed tag`
   reads your new version, and `Commit` reads the sha that built it.

Each app declares its own version in its own file, so bumping this one changes **only nodejswebapp**.
...
| | |
|---|---|
| this source | is the content of the Gitea repo **`demo/nodejswebapp-app`** |
| its manifests | `deploy/nodejswebapp/` -> the Gitea repo **`demo/nodejswebapp-deploy`** (ArgoCD's source) |

...
The **rust** app in this repo's six-language demo. All six serve an **identical page** — that is
...
```sh
cargo test      # tests
cargo build --release     # build -> target/release/rustwebapp
./target/release/rustwebapp       # run it, then open http://localhost:8080
```
...
```sh
make app-test  APP=rustwebapp
make app-build APP=rustwebapp
make app-run   APP=rustwebapp
```
...
1. In Gitea, open **`demo/rustwebapp-app`** -> **`Cargo.toml`** and bump the version
   (under `[package]`, NOT a `[dependencies]` entry):

   ```text
   version = "0.1.0"  # under [package]          ->   version = "0.1.1"  # under [package]
   ```
...
2. Commit. The push fires the webhook and the pipeline runs: clone -> test -> build -> push to
   Harbor -> write the new tag back into `rustwebapp-deploy`.

3. Watch it: the Tekton dashboard, then Harbor (`apps/rustwebapp` gains an artifact tagged with your new
   version **and** the commit sha, on one digest).

4. ArgoCD syncs the deploy repo and rolls the pod. Refresh **`rustwebapp.vks.local`** — `Deployed tag`
   reads your new version, and `Commit` reads the sha that built it.

Each app declares its own version in its own file, so bumping this one changes **only rustwebapp**.
...
| | |
|---|---|
| this source | is the content of the Gitea repo **`demo/rustwebapp-app`** |
| its manifests | `deploy/rustwebapp/` -> the Gitea repo **`demo/rustwebapp-deploy`** (ArgoCD's source) |

...
check-walk-env-manifest: ## Gate: every (scenario, key) the scenario docs name has a row in
	@#  docs/walk-env-manifest.tsv saying what the walkthrough harness does about it, and vice versa.
	@$(SCRIPTS)/check-walk-env-manifest.sh
...
.PHONY: check-count-fallback
check-count-fallback: ## Gate: a counting command (grep -c / wc -l) must not have a VALUE-EMITTING || fallback
	@$(SCRIPTS)/check-count-fallback.sh
...
install-all: $(INSTALL_ALL_STEPS) ## Run the complete air-gap install end to end (preflight FIRST, mirror integrity-verified, CI robot ensured BEFORE platform bakes the push/pull Secrets, then platform -> headlamp -> ingress -> gitops -> build-apps, so the demo actually SERVES)
	...
	@echo "                Supervisor Services) Harbor and ArgoCD."
	@echo ""
	@echo "  Your apps:    BUILT. build-apps pushed a commit per app, Tekton built each image"
	@echo "                into Harbor and wrote the tag back to <app>-deploy. ArgoCD rolls"
	@echo "                them on ITS reconcile timer, so for up to ~3 min a pod may still be"
	@echo "                coming up and its URL may 503. That window is expected; a 503 that"
	@echo "                PERSISTS past it is not."
	@echo ""
	@echo "  DO THIS NEXT: make verify    asserts the whole loop and the live pages (~3 min;"
	@echo "                               scenario-1 SS11). This is the ASSERTION, not the"
	@echo "                               build — the build already happened, above."
	@echo "  THEN:         make creds     endpoints + logins, with what each is serving."
	@echo "  ─────────────────────────────────────────────────────────────────────────────"
...
.PHONY: test-ci-pass-verdict
test-ci-pass-verdict: ## Unit-test ci-pass-verdict.sh's job lists against ci.yml (the sole required check's ONLY guard)
	@$(SCRIPTS)/test-ci-pass-verdict.sh
...
.PHONY: check-brew-lists
check-brew-lists: ## Gate: the macOS (Homebrew) package and gnubin lists, written in several places, cannot drift
	@$(SCRIPTS)/check-brew-lists.sh
...
.PHONY: static-check
.PHONY: check-jumpbox-shadow
check-jumpbox-shadow: ## Every Photon jump-box image pins /etc/shadow 0400 + asserts it at BUILD time
	@$(SCRIPTS)/check-jumpbox-shadow.sh

...
the step **above** say must not be used — an IP can only verify if the certificate carries an IP
SAN, and the default one does not. Follow that step — **“The ArgoCD address, in the ONE order
that works”** — rather than this paragraph: it is the one that makes the name resolve first.
...
```bash
make env-populate   # mints the Gitea secrets only, never a Harbor password (the one you were given stays); it CANNOT discover a Supervisor-hosted Harbor/ArgoCD from your GUEST kubeconfig (you set those above) — skip it if you set everything by hand
...
**Expect:** `make creds-show` prints a `Lab access` section — as a TENANT you supplied `VKS_USERNAME` and `SUPERVISOR_HOST` in `.env`, so the `vcf CLI` row carries YOUR values, not placeholders. This document never asks you for vCenter credentials, so unless you set them yourself the vCenter row reads `<not set>`.
...
Gitea, Tekton and every app row now shows a URL. If those names do not resolve on this machine yet, it
also prints an `/etc/hosts` line to add.

...
| Env | `creds-show` (alias `creds`) / `argocd-password` | Print the access URLs + logins / the ArgoCD admin password. `creds-show` **masks passwords when stdout is not a terminal** (a redirect, a pipe, a CI capture or the walk harness must not silently harvest them); `SHOW_SECRETS=1 make creds-show` forces the reveal. Note `argocd-password` is **not** guarded — it prints unconditionally, because `creds-show` reads it through a command substitution |

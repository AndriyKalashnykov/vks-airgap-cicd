...
| Mirror | `builder-image` | Build + push the deps-baked offline builder image for EVERY app (six today; ~4.3 GB, see [Sizing](sizing.md)) |
| Mirror | `builder-freshness` | **Read-only.** Report whether each app's builder image was built from THIS tree. A builder built from a different tree carries the SAME tag, so neither `podman images` nor the tag can tell you which one you are about to certify — this reads the `io.vks.builder.inputs` stamp instead. `BUILDER_FRESHNESS_ENFORCE=1` makes a stale or unstamped image fatal here and in `app-test`/`check-ui-contract`/`trivy-fs`/`app-run`. |
...
| Housekeeping | `prune-runs` | Reclaim completed Tekton PipelineRuns beyond the newest `PRUNE_KEEP` (default 3) **per app**, and with them their 2Gi workspace PVCs. Measured on a real lab after ONE DAY: 118 PipelineRuns / 466 TaskRuns / 118 PVCs = **236 GiB provisioned**, 118 of the cluster's 119 PVs; every `make verify` adds 12 GiB and nothing reclaimed it. Runs from the jump box using `tkn` + your own kubeconfig — no new mirrored image, no new RBAC — and WARNS rather than fails if the cluster refuses the delete (a scenario-2 tenant often cannot). |
...
| Trust | `vks-trust-probe` | LIVE, read-only: does this guest cluster trust our Harbor, and by what mechanism? Compares the CA fingerprint and forces a real pull, then classifies the failure — **TRUST (x509) and DNS are named separately**, because a DNS fault used to be reported as a trust problem. States in its own output what a green does **not** prove (it is not a DNS assertion). A tenant who cannot create a namespace re-runs it as `make vks-trust-probe PROBE_NS=<one they own>`; a namespace you supply is never deleted |

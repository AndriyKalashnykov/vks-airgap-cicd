...
| Offline build | one dependency-baked builder image **per language** (six) | Bakes each language's dependency cache (`~/.m2`, the Go module cache, `node_modules`, the venv, the cargo registry, the NuGet cache) so an in-cluster build reaches NO package registry |

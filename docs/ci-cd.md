...
| **static-check** | ⚠️ **`schedule` / `workflow_dispatch` ONLY** — measured 2026-09-08, `ci.yml:199`; it does **NOT** run on a PR, and **nothing invokes `static-check-pr`** (see B571) | `make static-check` — lint, manifest validation, fast script tests, and `sec`. ⚠️ **NOT `sec`** — trivy-fs and trivy-config do **not** gate a PR (`ci.yml:349` picks the target by `github.event_name`; `sec` is absent from `static-check-pr`, measured). **Weekly `schedule` only:** the full `make static-check`, which adds `sec`. gitleaks is unaffected — the always-on `secrets` job below runs it on every PR. |

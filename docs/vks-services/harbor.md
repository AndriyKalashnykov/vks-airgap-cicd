...
| `HARBOR_USERNAME` / `HARBOR_PASSWORD` | a **robot account**. `make harbor-robot` creates one **if you are a Harbor project-admin**; otherwise **request** it from the platform team | KinD-verified |

**Robot accounts are the tenant path.** A Harbor **project-admin** (granted directly, not via an SSO
group) can self-service a robot with push+pull on the two mirror projects — that is what
`make harbor-robot` does, writing `secrets/harbor-robot.env`. Without project-admin, this is a
...
> **Never run two registry-mutating operations at once.** Concurrent pushes corrupt Harbor's blob
> store: tags/manifests end up referencing blobs that `HEAD`-200 but are not stored, surfacing later
> as `MANIFEST_UNKNOWN` / `BLOB_UNKNOWN` on a pull or a Kaniko build — recoverable only by rebuilding
> the registry. This is now enforced mechanically (a `flock` guard on the mirror/builder pushes), not
> just documented. *(Learned the hard way: a second `e2e-kind` started while the first was finishing
> corrupted all 34 images.)*

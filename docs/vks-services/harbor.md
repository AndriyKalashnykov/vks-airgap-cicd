...
| CA propagation to the guest | **VKS copies Harbor's CA into the guest cluster itself** — `tkg-system/kapp-controller-config` key `caCerts` carries `CN = Harbor CA` with a SHA-256 **byte-identical** to our own `secrets/harbor-ca.crt` (`31:B4:DB:16:…:C0:4D`). Neither this repo nor `nested-vsphere-lab` writes it (measured: 0 hits for `additional-ca` in the lab repo, positive control passed). Verify on any lab with `make vks-trust-probe`. | **lab-verified 2026-08-21** [src: cmd="make vks-trust-probe" out="tkg-system/kapp-controller-config/caCerts  MATCH" date=2026-08-21] |
...
| CA trust for guest clusters (field path) | `osConfiguration.trust.additionalTrustedCAs[].caCert.{content,secretRef}` in the Cluster spec — published by a CAPI **runtime extension**, so it lives in the ClusterClass's `status.variables` and is **empty in `spec.variables`** (reading `spec` is what makes it look absent) | lab-verified [src: cmd="kubectl get clusterclass builtin-generic-v3.6.0 -n vmware-system-vks-public -o jsonpath='{.status.variables}'" out="osConfiguration -> trust -> additionalTrustedCAs[] -> caCert -> {content,secretRef}; spec.variables empty on all 12 ClusterClasses" date=2026-08-17] |
| CA **encoding** (secretRef path) | the value **VKS reads** (i.e. `.data` decoded once) must be the CA base64'd **once** — so a hand-written `data:` holds it **DOUBLE**-encoded, while `kubectl create secret --from-literal=` takes it **SINGLE**-encoded (kubectl adds the wire layer itself; measured `--dry-run=client`). ⚠️ Broadcom states this for the **v1beta1** API; **NOT ESTABLISHED for v1beta2**, which is what `k8s/vks/cluster.yaml` uses | 9.0-doc (v1beta1 only) [src: url=https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-service-administration-and-development/9-0/managing-vsphere-kuberenetes-service-clusters-and-workloads/provisioning-tkg-service-clusters/using-the-cluster-v1beta1-api/using-the-versioned-clusterclass/v1beta1-example-cluster-with-additional-trusted-ca-certificates-for-ssl-tls.html date=2026-07-19 quote="Double base64-encoding is required. If the contents are not double base64-encoded, the resulting PEM file cannot be processed."] |
| Ordering | trusting the CA can happen **at cluster creation or later** — the post-creation path is documented: `kubectl edit cluster/<name>`, add `osConfiguration.trust.additionalTrustedCAs`, applied on the next rolling update. Configuring Harbor first is operational convenience, **not a documented prerequisite** | 9.0-doc [src: url=https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-service-administration-and-development/9-0/managing-vsphere-kuberenetes-service-clusters-and-workloads/provisioning-tkg-service-clusters/using-the-cluster-v1beta1-api/using-the-versioned-clusterclass/v1beta1-example-cluster-with-additional-trusted-ca-certificates-for-ssl-tls.html date=2026-07-19 quote="When a rolling update is initiated for the cluster, the additional trusted CA certificates are added."] |
...
On a real lab the guest cluster's trust comes from the Cluster spec (`osConfiguration.trust.additionalTrustedCAs`,
...
- The remaining "community"-graded rows above (`tlsSecretLabels`, same-Supervisor auto-trust) —
  **verify on a real 9.1 lab**. The **double-base64 CA** and the **creation-or-later ordering** were
  ...
  rotate-by-NEW-Secret footgun) are saved in the private
  `claude-config/reference/harbor-on-vks.md` — this file grades facts, that file holds the artifact.
  ⚠️ `core.xsrfKey`'s **32-char** rule is a **Harbor app-config** requirement (already `8.0-doc`), NOT
  a constraint on the trusted-CA Secret's data key — two different "keys"; do not conflate them.

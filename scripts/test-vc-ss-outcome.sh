...
# shellcheck disable=SC2016  # a LITERAL source line to find; nothing should expand
if grep -qF '|| [ "${VC_SS_OUTCOME:-}" != installed ] || state_set HARBOR_PASSWORD' scripts/04-install-harbor-service.sh; then

...
pom="$(grep -oE '<java\.version>[0-9]+' apps/java/javawebapp/pom.xml | grep -oE '[0-9]+' | head -1 || true)"
[ -n "$pom" ] || { echo "ERROR: could not read <java.version> from apps/java/javawebapp/pom.xml" >&2; exit 1; }

drift=0
check() { # <label> <actual-major> — compare to $pom
  if [ "$2" != "$pom" ]; then
    echo "DRIFT ${1}: Java ${2:-<none>} vs apps/java/javawebapp/pom.xml=${pom}"
    drift=1
  else
    echo "ok    ${1}=Java ${2}"
  fi
...
df_build="$(grep -oE 'BUILDER_IMAGE=maven:[0-9.]+-eclipse-temurin-[0-9]+' apps/java/javawebapp/Dockerfile | grep -oE '[0-9]+$' | head -1 || true)"
df_run="$(grep -oE 'RUNTIME_IMAGE=eclipse-temurin:[0-9]+' apps/java/javawebapp/Dockerfile | grep -oE '[0-9]+$' | head -1 || true)"
...
check "apps/java/javawebapp/Dockerfile BUILDER_IMAGE (maven)" "$df_build"
check "apps/java/javawebapp/Dockerfile RUNTIME_IMAGE"         "$df_run"

...
want() { # want <label> <expected> <version>...
  ...
  [ "$got" = "$exp" ] && ok "$label" || bad "$label" "want '$exp', got '$got'"
}
...
[ "$(printf '%s\n' 1.9.0 1.100.0 | jq -R -s -r '[splits("\n")|select(length>0)]|sort|last')" = 1.9.0 ] \
...
[ "$(printf '%s\n' 1.28.5 1.28.5-rc1 | jq -R -s -r "$_VKEY"' [splits("\n")|select(length>0)]|sort_by(vkey)|last')" \
  != "$(printf '%s\n' 1.28.5 1.28.5-rc1 | sort -V | tail -1)" ] \
...
[ "$n" = 2 ] && ok "both selection sites use the shared vkey (2 uses)" \
...
[ "$got" = a ] && ok "a Package with no .spec.version is skipped, not fatal" \

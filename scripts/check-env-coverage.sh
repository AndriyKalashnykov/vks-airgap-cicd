...
# .env.example.
# TERM / NO_COLOR joined the standard-environment class (PATH/HOME/SHELL/DISPLAY/...) on
# 2026-09-10, when creds.sh's ANSI gate began reading them. They are set BY THE TERMINAL, not
# by an operator, so `.env.example` is the wrong home twice over: nobody configures them there,
# and `load_env` sources that file with `set -a`, so an uncommented TERM would be EXPORTED to
# every child process -- handing the whole run a bogus terminal type. NO_COLOR is a cross-tool
# convention (no-color.org) -- but it is an OPERATOR KNOB, not terminal environment: any
# non-empty value CHANGES this repo's output, so it is DOCUMENTED in .env.example rather than
# exempted. My first pass exempted it on a FALSE DICHOTOMY -- I argued an uncommented TERM
# would be exported by `set -a`, against a form nobody proposed: this gate accepts a COMMENTED
# slot (`^#?[[:space:]]*${v}=`, :185) and a comment is never sourced. Refuted by
# adversary-bash-git-cli 2026-09-10.
# HTTPS_PROXY, HTTP_PROXY, NO_PROXY: the operator's ambient shell proxy (like PATH/HOME/SSL_CERT_FILE above), never a repo
# tunable. creds.sh only READS it to interpret a kubectl refusal — a dead local proxy prints the same terse
# "…was refused" line as a refusing cluster (2026-09-15, measured with real kubectl).
# MAKE / PLATFORM_REPORT_MAKE_VERSION: set BY the `make platform-report` recipe from make's own
# $(MAKE)/$(MAKE_VERSION), so the report names the make that RUNS it. PRETTY_NAME: /etc/os-release.
# CONTAINERS_CONF: podman's OWN variable; lib/rosetta.sh only reads it to stand aside when it is set.
# STATE_ARCHIVED_AS: set BY state_archive (lib/state.sh) so a caller can print the undo; never an input.
INTERNAL='ARGOCD_ENDPOINT_HTTP|ARGOCD_ENDPOINT_RC|HARBOR_FIRST_INSTALL|HARBOR_SETTLE_FRESH|ARGOCD_SERVER_SOURCE|PKG_VER_RESOLVED|PKG_NS_RESOLVED|VKS_SUDO_PROBED|BUILDER_IMAGE_TAG_DEFAULT|VKS_STATE_KIND|VKS_LAB_STATE_DIR|SERVICE|PACKAGE|APP_NAME|APP_LANG|APP_SRC|APP_DEPLOY_DIR|APP_HOST|APP_TEST_TASK|APP_NAMESPACE|APP_GIT_REPO|APP_DEPLOY_REPO|APP_IMAGE|APP_BUILDER_IMAGE|APP_RUNTIME_IMAGE|APP_HOSTS_BLOCK|APP_NS_BLOCK|PROBE_HOST|PROBE_APP|APP_ING_ALLOWLIST|REPO_ROOT|SCRIPT_DIR|BASH_SOURCE|PATH|HOME|PWD|IFS|SHELL|ZDOTDIR|BASH_VERSION|HOMEBREW_PREFIX|SSL_CERT_FILE|GODEBUG|ROSETTA_PROBE_BIN|XDG_CONFIG_HOME|TMPDIR|LC_ALL|HARBOR_PW|HARBOR_SVC|HARBOR_TMP|HARBOR_CURL_CFG|HARBOR_CODE_FILE|HARBOR_TMP_DIR|HARBOR_RELEASE|HARBOR_TLS_SECRET|HARBOR_TLS_VERIFY|HARBOR_INSECURE_BOOL|HARBOR_PROVISIONAL_EXTERNAL_URL|HARBOR_ROBOT_OUT|ARGOCD_SVC|ARGOCD_NS|ARGOCD_API|GUEST_API|GITEA_CLONE_URL|GITEA_ARGOCD_URL|ARGOCD_DEST_KEY|ARGOCD_DEST_VALUE|KIND_KUBECONFIG|KIND_CLUSTER_REMOVED|ARGOCD_MANAGER_NS|ARGOCD_MANIFEST_VERSION|ISTIO_ROUTE_API_EFFECTIVE|PLATFORM_ISTIO_NAMESPACE|PLATFORM_ISTIO_RELEASE|PLATFORM_ISTIOD_NAMESPACE|PROBE_IMAGE|REGISTRY_LOCK_FILE|MANIFEST_DIR|DOCKER_HOST|DOCKER_CONFIG|XDG_RUNTIME_DIR|ENGINE_SUDO_COUNT_FILE|CERTD|JUMPBOX_[A-Z_]*|E2E_[A-Z_]*|CI|GITHUB_[A-Z_]*|READY_TIMEOUT_SECONDS|POLL_INTERVAL_SECONDS|CURL_MAX_TIME_SECONDS|MIRROR_RETRIES|MIRROR_FORCE_PULL|NOTIFY|VCF_[A-Z_]*|PSA_LEVEL_[A-Z_]*|DISPLAY|WAYLAND_DISPLAY|TERM|DRY_RUN|GW_IP|TOKEN|RED_TEST_SKIP_PRECHECK|CREDS_TOKEN|GATEWAY_IMAGE_FIXTURE|PODIMAGES_FIXTURE|PODIMAGES_MATCHED_VIA|BUNDLE_HOST_ARCH|BUNDLE_CRANE_VER|BUNDLE_MIRROR_ARCH|VC_TOKEN_FILE|VC_CURL_CFG|VC_HDR_FILE|VC_CODE_FILE|VC_SS_OUTCOME|SVC_STATUS|CA_STATUS_CHECKED|CA_STATUS_MATCHED|CA_STATUS_STRICT|PF_PID|PF_GEN|PF_DEATHS|PF_LAST_BODY|PF_RESTARTS_BLOCKED|TREE_STABILITY_ID|VKS_VIP_STILL|MIRROR_REGISTRY_HOSTS|HOSTSCAN_ALLOW|HTTPS_PROXY|NO_PROXY|HTTP_PROXY|MAKE|PLATFORM_REPORT_MAKE_VERSION|PRETTY_NAME|CONTAINERS_CONF|XDG_STATE_HOME|STATE_ARCHIVED_AS'

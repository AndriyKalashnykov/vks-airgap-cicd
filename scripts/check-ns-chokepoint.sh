...
scripts/46-install-istio.sh|3|two helm create-ns flags for the mesh namespaces, plus one explicit form for the GATEWAY ns. It must NOT route through the chokepoint: that stamps istio-injection=disabled (right for app namespaces, B26) and the placeholder image in the gateway chart is rewritten ONLY by the injection webhook -> measured 2026-08-10, the pod stuck in ImagePullBackOff. PSA labels are applied by psa_label_namespace BEFORE the installs, because the helm flag makes an UNLABELLED ns and VKS enforces restricted by default

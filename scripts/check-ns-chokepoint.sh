...
scripts/43-install-istio-package.sh|2|the SAME two mesh namespaces as 46, for the SAME reason and by the same explicit form. The package path must create and PSA-label them BEFORE kapp-controller schedules into them (it creates them UNLABELLED otherwise), and neither may route through the chokepoint: it stamps istio-injection=disabled, which two controls say must never touch the mesh namespaces (lib/psa.sh, check-namespace-labelled.sh), and which defeats the injection webhook that rewrites the gateway image placeholder -- measured 2026-08-10 as ErrImagePull on a literal auto.

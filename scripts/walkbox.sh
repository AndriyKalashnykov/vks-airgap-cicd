...
walkbox_run() {
  ...
  _ssh "$ip" "set -a; . \$HOME/.walk-secrets; set +a; rm -f \$HOME/.walk-secrets; \
      ...
      VKS_STORAGE_POLICY='${VKS_STORAGE_POLICY:-}' VKS_AUTH_METHOD='${VKS_AUTH_METHOD:-}' \
      ...
      WALK_DOC=/tmp/doc.md WALK_EXISTS='${WALK_EXISTS:-1}' WALK_ROBOT_EXISTS='${WALK_ROBOT_EXISTS:-1}' WALK_ISTIO='${WALK_ISTIO:-existing}' WALK_OS='${WALKBOX_OS:-ubuntu}' \
      WALK_NS_EXISTS='${WALK_NS_EXISTS:-}' WALK_HARBOR_EXISTS='${WALK_HARBOR_EXISTS:-}' WALK_ARGOCD_EXISTS='${WALK_ARGOCD_EXISTS:-}' WALK_CLUSTER_EXISTS='${WALK_CLUSTER_EXISTS:-}' \

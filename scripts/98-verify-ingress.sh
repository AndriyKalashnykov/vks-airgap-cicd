...
if [ "$rc" -ne 0 ]; then
  ...
  case "${INGRESS_CONTROLLER}" in
    istio|istio-existing)
      ...
      log_error "    connection refused / no HTTP at all -> TWO causes, and they need different commands."
      log_error "      (a) the DATA PLANE is not serving: the proxy exists but is not Ready. This is the"
      log_error "          one to check FIRST on the gateway-api path, because Istio programs the Gateway"
      log_error "          and publishes an address regardless (measured: B477 -- Programmed=True with an"
      log_error "          LB IP while the proxy CrashLoopBackOff'd because istiod's Service had ZERO"
      log_error "          endpoints, so its selector matched no pod)."
      log_error "            kubectl -n ${ISTIO_GWAPI_NAMESPACE:-vks-ingress} get pods -l gateway.networking.k8s.io/gateway-name=${ISTIO_GATEWAY_NAME:-vks-uis}"
      log_error "            kubectl -n <istio-ns> get endpoints istiod     # <none> = its selector matches NO pod"
      log_error "      (b) the CLASSIC path only: the Gateway's selector matches no gateway workload. The"
      log_error "          Gateway object is accepted without error, so nothing flags it. Compare:"
      ...
      log_error "      NOTE: 'make istio-preflight' answers (b) ONLY. It exits 0 with PREFLIGHT OK on the"
      log_error "      gateway-api path -- it returns before it ever looks at a gateway workload -- so a"
      log_error "      green preflight does NOT rule out (a). That misdirection is what cost 40 minutes."

...
ALLOWLIST='${CI_NAMESPACE} ${HARBOR_URL} ${HARBOR_INFRA_PROJECT} ${APP_BRANCH} ${APP_REPO_CLONE_URL} ${DEPLOY_REPO_CLONE_URL} ${ARGOCD_TRACK_BRANCH} ${APP_IMAGE} ${APP_NAME} ${APP_TEST_TASK} ${APP_GIT_REPO} ${APP_BUILDER_IMAGE} ${APP_RUNTIME_IMAGE} ${APP_BUILD_ARGS} ${APP_VERSION_CMD_B64} ${HARBOR_INSECURE_BOOL}'

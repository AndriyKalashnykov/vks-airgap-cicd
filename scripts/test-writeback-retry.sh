...
echo "== race, upstream names a commit, and the APP repo cannot be fetched: must REFUSE, not overwrite =="
mkdeploy; prepare; compete "$C3"; git -C "$W/app" remote set-url origin "file://$T/does-not-exist.git"; runstep "$step"
if [ "$R" != 0 ] && [ "$(remote_commit)" = "$C3" ] && printf '%s' "$O" | grep -q 'refusing to overwrite'; then ok "fetch failure -> refuses; ${C3} stays deployed"
else bad "fetch failure: rc=$R remote=$(remote_commit) $O"; fi

echo "== two more competing pushes arrive during our retries: the loop still lands ONE commit of ours =="
# A git shim: before our 1st and 2nd push, another (older-commit) run pushes first, so we are
# rejected twice and succeed on the 3rd attempt — the second and third loop iterations run.
REAL_GIT="$(command -v git)"
mkdir -p "$T/shim"
cat > "$T/shim/git" <<SHIM
#!/bin/sh
if [ "\$1" = push ]; then
  n=\$(cat "$T/pushes" 2>/dev/null || echo 0); n=\$((n+1)); echo \$n > "$T/pushes"
  if [ "\$n" -le 2 ]; then
    rm -rf "$T/rival"; "$REAL_GIT" clone -q "$T/deploy.git" "$T/rival"
    "$REAL_GIT" -C "$T/rival" commit -q --allow-empty -m "rival \$n"
    "$REAL_GIT" -C "$T/rival" push -q origin HEAD:main
  fi
fi
exec "$REAL_GIT" "\$@"
SHIM
chmod +x "$T/shim/git"
mkdeploy; prepare; rm -f "$T/pushes"
PATH="$T/shim:$PATH" runstep "$step"
ours="$(git --git-dir="$T/deploy.git" log --format=%s main | grep -c '^ci: deploy app 0.1.0')"
if [ "$R" = 0 ] && [ "$(remote_commit)" = "$C2" ] && [ "$ours" = 1 ] && [ "$(cat "$T/pushes")" = 3 ]; then ok "rejected twice, written once on the 3rd push"
else bad "multi-retry: rc=$R remote=$(remote_commit) ours=$ours pushes=$(cat "$T/pushes" 2>/dev/null) $O"; fi

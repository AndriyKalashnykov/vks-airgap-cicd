...
| **B26** | 🔴 **Fixes (1)(2)(4) SHIPPED (#295/#296); this row is now fix (3) ONLY.** The pod templates carry `sidecar.istio.io/inject: "false"` (all 4, `check-pod-inject-label` green); `ensure_namespace` stamps `istio-injection=disabled` (`psa.sh:174`); the attach e2e now injects, not simulates the safe case (shipped #296). **Fix (3) UNBUILT, but its INVARIANT IS NOW MEASURED (2026-07-18) — build the gate to this spec, do not re-derive it.** An **offline `helm template` gate** in `static-check` renders the carried istiod chart under all 3 configs and asserts every webhook rule is defeated by BOTH our ns-label and our pod-label. Measured on the carried `istiod-1.30.3.tgz`, offline, no cluster: **base → 4 rules, `enableNamespacesByDefault=true` → 5, `revision=canary` → 2** (see B35). Across **all** of them, EVERY rule is defeated **twice over, independently** — by `istio-injection=disabled` on the namespace AND by `sidecar.istio.io/inject="false"` on the pod: `rev.namespace`/`rev.object`/`auto` die on `istio-injection DoesNotExist` (we set it) and on the pod key's `NotIn ["false"]`/`DoesNotExist`; `namespace` dies on `In ["enabled"]` (ours is `disabled`); `object` dies on `In ["true"]` (ours is `"false"`). That double coverage is the property the gate must assert — losing ONE of the two labels must still leave the mesh unable to inject, and the gate's RED-proof is to drop one label and watch the other still hold, then drop both and watch it fail. ⚠️ **Lab-gated residual:** the shipped fixes are upstream **1.30.3**-rendered; VKS ships `1.28.2+vmware.1-vks.1` [community] and Broadcom could patch the injector template. One command settles it: `kubectl get mutatingwebhookconfiguration -o yaml \| grep -A8 namespaceSelector`. |
...
| **B46** | 🔴 **NEW 2026-07-18, needs an OWNER DECISION: the repo defaults to podman, and that DOUBLES the carried builder.** `test-builder-save-crane.sh` claimed "both `podman save` and `docker save` … default to the SAME docker-archive format — so there is NO format divergence to catch". **Measured false** (same Dockerfile, same source, back to back): `docker save` 29.6.2 → **oci-layout** + `index.json` + `blobs/sha256/`, layers **gzip** (`1f 8b`), **286,969,856 B**; `podman save` 4.9.3 → `<hash>.tar` **docker-archive v1**, layers **plain tar** (`65 74`), **623,348,736 B** — **2.17×**. (The podman rebuild landed byte-identical in size to a pre-existing artifact of unrecorded provenance, retroactively identifying that one as podman-produced.) Both remain crane-pushable, so the *outcome* stood — but the stated *reason* was false, and it told readers not to look. Why it survived: a successful `crane push` proves crane handles **both** and says nothing about them being the **same**; the discriminating command was `tar tf`. The honesty block is corrected in-place. **The open question is operational, not a bug:** should `14-builder-build.sh` prefer docker (or pass `--format oci` / compress) so the sneakernet bundle carries ~336 MB less per builder? That trades the repo's sudo-free podman default against bundle weight — **an owner call**, and it interacts with the "docker is opt-in, never required" invariant (`test-container-engine.sh` check 7). Do NOT change the default unilaterally. |

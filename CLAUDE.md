...
| `make static-check` | Composite offline code gate — the **authoritative** prereq list is the `static-check:` line in the Makefile (alignment + doc/terminology gates + env/app gates + `lint` + `validate` + `sec` + `test-scripts`; NOT `app-test`, which is in `app-verify` and deliberately out of CI). Do NOT re-enumerate it here — a hand-typed subset rots on the first Makefile edit. |
...
| `make app-test` / `app-run` / `app-verify` | Build/test **every** app (java: `./mvnw`, go: `go test`/`go build`); one app: `APP=javawebapp\|gowebapp` (`app-run` defaults to javawebapp). Apps are the rows of `apps/registry.tsv`. |
...
| `make install-all` | Full air-gap install: `preflight → selfbuilt-image → mirror → mirror-verify → builder-image → vks-login → harbor-robot-ensure → platform → install-headlamp → install-ingress → gitops → build-apps`. `preflight` runs FIRST and is read-only — it stops a 20-min mirror on a box that can't finish; `mirror-verify` is the blob-integrity gate; `build-apps` is what makes the demo actually SERVE. |
...
- **Scaleway Mac** `m1@51.159.120.46`: after the fresh-Mac walk it holds only what the doc installs
  (Homebrew base line, colima/docker/buildx/dmnc, the mise toolchain), a walk clone `~/fresh/vks-airgap-cicd`,
  and the old clone `~/vks-airgap-cicd` with its 8.2 GB `bundle/` (public images, kept for B735). Colima
  stopped, no podman, no clusters, no daemon. `.zshrc` was regenerated by `gmake shell-init`.
- **The pre-wipe backup was DELETED 2026-09-26** (security review: nothing in it was needed — shell
  config and the Rosetta drop-in regenerate, the VM state pointed at deleted VMs, `~/.kube` held a stub).
  Its one unique item, a plaintext `u@localhost:5000` docker login of unknown origin with no consumer
  in the repo, was not restored; live config has 0 `localhost:5000` entries; no local snapshots. The
  non-secret listing and pre-state are in `~/walk-evidence/from-mac/wipe-20260926/` (0600). If the
  owner ever recognises that password, rotate it where it lives.

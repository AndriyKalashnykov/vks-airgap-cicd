...
**State: `main` GREEN once the s3 prune PR lands, tree clean, everything pushed. NOTHING is half-done.**
**This session (s3) shipped NO code — it was a docs/backlog prune + one idea-round.** It confirmed
**B29, B30(a/b), F8, B41, B44** were already SHIPPED in #295–#297 (the backlog still listed them OPEN
— two consecutive picks landed on already-done rows, so the backlog itself was the bug), removed those
five from the backlog, collapsed **B30/B26/B33** to their open residuals, folded B23 into B22, and ran
the **B37 idea-round** (below). **No cluster is up** — the prior `kind-down` was verified BY ARTIFACT
(0 clusters, 0 `kindccm-*` orphans, 0 cloud-provider-kind), it deleted only what it created and
...
- **`make e2e-kind` SILENTLY REUSES a cluster** (B42). A second run tests an idempotent re-apply
  where create-ordering CANNOT fail. Only `kind-down` first is evidence.
...
- **B30c is the cheapest REAL work and is now buildable HONESTLY** — a unit test of the SHIPPED gates:
  delete a real `ensure_namespace` CALL → assert `check-namespace-labelled` reddens; delete a
  pod-template label → assert `check-pod-inject-label` reddens; plus `psa_label_namespace`'s untested
  branches. RED-provable WITHOUT the refuted `make red-prove`, because deleting a real call IS mutating
  the shipped product — which is exactly what B37's incident got wrong (it mutated the helper).
- **B34 + B42** — `make kind-down && make e2e-kind` cold: the only real-cluster evidence that the
  now-fixed B29 ordering holds end to end. Heavy (a full re-mirror).
- **B37 is REFUTED — do NOT rebuild `make red-prove`** (idea-round this session; flip-detection is
  ...
  row). **B39** (an instrument must reproduce a known answer first) still needs its OWN idea-round.
  **B45** (the gate's time-vs-content re-arm) is real but must NOT be a content-scoped receipt (refuted
  2026-07-16).
...
(session 3)**, when **B29, F8, B41, B44 and B23 were REMOVED** because they shipped/resolved
(#295–#297, evidence in each PR + the code), and **B30, B26, B33 were COLLAPSED** to their open
residuals (their done parts dropped). The history that produced these is in git and `docs/reviews/`;
...
| **B30** | 🔴 **(a) + (b) SHIPPED (#295, `0a1d077`); this row is now (c) ONLY.** `check-namespace-labelled.sh` (the chokepoint gate — keyed on the psa-check INVENTORY, not syntax; the syntax form was refuted, see B40) and `check-pod-inject-label.sh` are both in `static-check` and green (`check-pod-inject-label`: **4/4**, 0 exempt — the old "3 of 4, traefik doesn't" is STALE; traefik carries it at `k8s/traefik/controller.yaml:77`). **(c) `test-ensure-namespace-labels.sh` still does not exist and is BLOCKED:** its naive form is exactly the blind test B37 refutes (RED-proving by mutating the helper, not the missing CALL). The HONEST buildable version tests the SHIPPED gates directly — delete a real `ensure_namespace` CALL (e.g. `40-install-gitea.sh:56`) → assert `check-namespace-labelled` reddens; delete a pod-template label → assert `check-pod-inject-label` reddens; plus `psa_label_namespace`'s untested branches (invalid level dies; `none`; empty). RED-provable WITHOUT the refuted `make red-prove`, because deleting a real call IS mutating the shipped product. |
...
| **B33** | **Loose ends from the 2026-07-17 rounds — (a)(b) DONE, (c)(d)(e) OPEN (verified 2026-07-17 s3).** ✅ (a) the dead `: "${ARGOCD_NAMESPACE:?}"` is gone from `45-install-traefik.sh` (a comment records why). ✅ (b) `90-e2e-istio-existing.sh:257` now uses `ensure_namespace "$PROBE_NS"` (the bare creates at :170/:196 are the deliberate bare-ns CONTROL cells). ⚠️ (c) `22-harbor-robot.sh`'s `mkdir -p` (:144) is still outside the `umask 077` subshell (:148) — the FILE is 0600 (a `rm -f` precedes it) but `secrets/` is 0775; a dirname is not a secret, so minor. 🔴 (d) `scenario-2.md` still restates `robot$<name>` (:87, :283) — reference `secrets/harbor-robot.env`, not the literal (the "credentials in docs must REFERENCE their single source of truth" rule). 🔴 (e) `test-classify-changes.sh` has no `.claude/**` case — a hook / `settings.json` change could route to the docs-only path and skip `static-check`, the one control that enforces RULE ZERO. |
| **B34** | **`make e2e-kind` has NOT been run against #290's `ensure_namespace` change.** The owner explicitly asked for e2e verification on the Harbor credential work; PR-A then shrank to a 2-char doc fix and the requirement was dropped — but **#290 went on to change `ensure_namespace`, which EVERY namespace creation flows through**, and shipped on `static-check` alone. It is the riskiest thing merged that day and the least verified. Run it — but per **B42** a WARM `make e2e-kind` silently reuses a cluster and CANNOT prove create-ordering, so this must be `make kind-down && make e2e-kind` (cold). |

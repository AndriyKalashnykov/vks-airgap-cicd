...
| **B49** | 🔴 **TWO GATES ARE BLIND TO AN EMPTY CORPUS — reproduced 2026-07-18, rc=0 over zero content.** Empty every tracked `.md` (leaving them tracked) and: `check-doc-novels` → `OK — scanned 21 doc(s)` **rc=0**; `check-doc-robot-quoting` → `OK — scanned 22 file(s)` **rc=0**. Their zero-guards fire on the **FILE** count, which is healthy, while the **ITEM** count that carries the meaning is zero. `check-doc-make-targets` is worse in one way and better in another: it prints both (`0 command(s) across 25 doc(s)`, rc=0) *and* `:40` deliberately `exit 0`s when no docs match. **The fix is a DESIGN QUESTION, not a typo** — for these gates the items are *violations*, and zero violations is the SUCCESS condition, so the honest denominator is "candidate lines/blockquotes EXAMINED", plus a decision about what minimum is suspicious versus legitimate. Do not paper over it with `checked 1`. Reproduce with: `git archive HEAD \| tar -x -C $T`, re-init git there, `git ls-files '*.md' \| while read -r f; do : > "$f"; done`, run the gate, **read its own rc on its own line** (a `\| tail` in the harness reports tail's status — that happened while finding this). |

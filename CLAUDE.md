...
| **B52** | 🔵 **60 `printf '%s' "$var" | grep -q` sites remain UNCONVERTED — bounded, but NOT safe in principle.** The SIGPIPE class was root-caused and fixed 2026-07-19 (#357) for the **unbounded** case (a producer reading a FILE), and `make check-grep-q-pipe` gates that. What is left is the bounded form: `grep -q` still exits at the first match, and **bash forks the builtin into a subshell that takes the signal** — measured: `printf "$page" | grep -q` on a large HTML body returns **rc=141**, which is why `99-verify.sh`'s marker check (a whole rendered page) was one of the 12 fixed. The 60 remaining are mostly a password, a hostname, a short status string, where the producer finishes before grep can leave; the exposure is real but small. **Counted, and scanned for the dangerous subset:** a sweep for a piped variable filled from a large producer (curl/kubectl/docker/helm/logs) among those 60 returned **ZERO** — the only two genuinely large ones in the repo (a rendered HTML page in `99-verify.sh`, a cloud-provider-kind log in `05-kind-up.sh`) were both in the 12 already fixed. ⚠️ That scan is a crude heuristic on assignment shape, not a proof; it lowers the priority, it does not close the row. ⚠️ **Do NOT gate this by adding a `# grep-q-ok:` marker to all ~70** — that is the wrong-shape signal (70 exemptions on day one) that killed an earlier design in B40. Either convert them all mechanically (`grep -q PAT <<< "$v"`, measured 400/400 clean) and then widen the gate to ban `\| *grep -q` outright, or leave them and keep the gate's honest SCOPE line. The conversion is mechanical but touches ~30 files, so it wants its own PR and its own RED-proof per changed gate — a botched sweep here would be a false green in a *test*, which is the worst place for one. |

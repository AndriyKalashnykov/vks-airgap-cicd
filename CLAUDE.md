...
holds on a real cluster), both apps' GitOps pipelines SUCCESS, mirror 36/36, ingress reachable. **B30 and B34 are now DONE;
B42's own fix (the default target reuses silently) is still open.** **No cluster is up** — the e2e
cluster was torn down + verified BY ARTIFACT (0 clusters, 0 `kindccm-*` orphans, 0
...
- **B42** — the cold e2e was RUN this session (B34 closed, GREEN — B29 ordering proven on a real
  cluster), but B42's own fix is still open: the DEFAULT `make e2e-kind` silently reuses a cluster, so
  it can't prove create-ordering. Fix = make the reuse LOUD (`log_warn` it can't prove ordering) or add
  an `E2E_FRESH=1` cold mode. Small Makefile/script change.
...
(session 3)**, when **B29, F8, B41, B44, B23 and B30 were REMOVED** because they shipped/resolved
(#295–#297 + #304, evidence in each PR + the code), and **B26, B33 were COLLAPSED** to their open
...
| **B42** | ⚠️ **The COLD run was DONE 2026-07-17 s3 (B34 closed): `make kind-down && make e2e-kind` GREEN end to end** — `PSA OK, 7 namespaces measured` (gitea/tekton/apps=restricted, ci/istio=baseline, all labelled at creation on a FRESH cluster → the B29 ordering fix holds on a real cluster), both apps' GitOps pipelines SUCCESS, mirror 36/36, ingress reachable, teardown verified by artifact. **But B42's OWN fix is STILL OPEN — the DEFAULT `make e2e-kind` still SILENTLY REUSES an existing cluster** (I ran cold by hand; nothing in the target forces it), so a second run cannot catch a create-ordering regression, and it looks identical to a real pass. MEASURED 2026-07-17: a re-run printed `kind cluster 'vks-airgap-cicd' already exists and is healthy — skipping create` and then tested an idempotent re-apply over already-created, already-labelled namespaces — precisely the condition under which `ensure_namespace`-before-the-apply CANNOT fail (the namespace already exists, so a missing or late call is invisible). Its preflight `psa-check` also measured 7 instead of 0, because it was seeing the PREVIOUS run's leftovers. Only `make kind-down && make e2e-kind` is evidence for ordering. `e2e-kind-istio-existing` already does `kind-down` first, deliberately, with a comment explaining why — so **the DEFAULT e2e is the weaker of the two and nothing says so**. Same class as F2 itself: the test's own setup is the camouflage. Fix: either kind-down first (costly — a full re-mirror), or make the reuse LOUD (`log_warn` that this run cannot prove create-ordering and to re-run cold for that), or add a `E2E_FRESH=1` mode the docs name. Do not leave it silent. |

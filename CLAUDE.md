...
| **B50** | **Route-verify the platform hosts on the air-gap leg — currently NOT verified there, and the two obvious fixes are both bad.** B3's block installs the ingress but does not route-check it, because `98-verify-ingress.sh:79` asserts over a FIXED host set — `$GITEA_HOST $TEKTON_DASHBOARD_HOST $APP_HOSTS` — and the app hosts' backends are deployed by ArgoCD via `gitops`, which B3 deliberately excludes. **Measured on the air-gap box:** gitea `200`, tekton `200`, javawebapp `503`, gowebapp `503`. So the ROUTING evidence exists; only the gate is mis-scoped for this context. ⛔ **Do NOT add a host-subset knob** (`VERIFY_INGRESS_HOSTS=…`): narrowing a verification gate to obtain green is the anti-pattern this repo exists to prevent, and it would let any future caller silently shrink the assertion. ⛔ **Do NOT pull in `gitops`** just for this — it needs ArgoCD (never installed by `e2e-sneakernet.sh:112`) and roughly triples the leg. **A third option worth an idea-round:** have the script SKIP a host whose backend workload does not exist, reporting it LOUDLY with that reason — honest and self-adjusting, but it changes a verification gate's semantics and risks the passes-by-not-looking failure, so it needs its own adversary round before anyone builds it. |

...
**State: `main` GREEN @ `de77d5d`, tree clean, everything pushed. NOTHING is half-done.**
...
green). **B30 is now fully DONE.** **No cluster is up** — the prior `kind-down` was verified BY ARTIFACT
...
- **B34 + B42** — `make kind-down && make e2e-kind` cold: the only real-cluster evidence that the
  now-fixed B29 ordering holds end to end (a WARM re-run reuses the cluster and cannot prove ordering —
  B42). Heavy (a full re-mirror). This is the next real item now that B30c has shipped.

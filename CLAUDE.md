...
**State: this repo `main` GREEN @ `#325`, tree clean. `claude-config` `main` GREEN, 8 PRs merged
...
| **B18** | 🔒 **BLOCKED ON A PERMISSION, not on effort** (= `claude-config` HOOK-004). The subagent-readonly hook's worktree exemption keys on the **target path**, not the owning agent — a subagent could write into *another* agent's worktree by absolute path. A cwd-anchored fix needs a live `getcwd()` probe from a real `isolation:"worktree"` subagent; `hooks.md` is explicit that assumptions about harness timing/cwd are unreliable and only the instrumented test settles it. **Attempted 2026-07-18 and refused by the safety classifier** — the probe requires writing a temporary trace into the LIVE global hook (`~/.claude/hooks/subagent-readonly.py`), which is correctly treated as tampering with a security control. I did not work around it. **To unblock, the operator grants a one-off Bash permission for that path;** the procedure is then ~5 minutes: (1) `cp` the hook to a backup, (2) insert a trace at the top of `_decide()` logging `os.getcwd()` + `agent_id` + `tool_name` to `/tmp/hook-cwd-probe.log` (the hook script is re-read from disk on EVERY invocation, so no restart is needed), (3) dispatch any `isolation:"worktree"` subagent that runs one Bash command, (4) read the log — if `cwd` is the agent's own `.claude/worktrees/<seg>/`, a cwd-anchored exemption is safe; if it is the caller's repo root, it is NOT and the current target-path keying must stay, (5) restore from the backup and verify `--selftest` is green. **Do not "fix" this on an assumption** — a wrong guess false-blocks legitimate worktree writes, which is how a gate gets ripped out. Blunted meanwhile by worktree isolation itself. |

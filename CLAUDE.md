...
| **B48** | **`return $rc` in an EXIT trap does not set the exit status — right answer, wrong reason.** `scripts/e2e-sneakernet.sh:70`. In bash the shell's exit status is preserved across an EXIT trap unless the trap itself calls `exit`; `return` from the trap function does NOT set it. So the status is correct — for a different reason than the code implies. Harmless here, misleading to the next reader who copies the idiom somewhere it matters (a trap that DOES need to override the status needs an explicit `exit $rc`). Fix: comment it, or drop the `return`. LOW. |

...
not "and a mirrored Harbor" — a small doc-fix worth doing. **B30, B34, B42, B16, B33e are now DONE.** **No
...
- **Harbor-credential cluster (B13/B14/B31/B32 + B33d) — DEFERRED to a focused session (risky, needs an
  e2e-kind verification run).** s3 investigation resolved "is B14 live?": `.env` beats `.env.example`, so a
  tenant who sets `HARBOR_USERNAME` is fine — the footgun is *password-only* (the operator gets the
  uncommented `HARBOR_USERNAME=admin` + their robot secret → 401). The fix (comment `HARBOR_USERNAME` in
  `.env.example`) BREAKS the `e2e-kind` SKIP_DOTENV path unless `05-kind-up` publishes it, which reopens the
  MEASURED `.env.state` clobber trap. That interlock + heavy prior debate is why it is not a quick fix.
...
| **B33** | **Loose ends — (a)(b)(e) DONE, (c) WON'T-FIX, only (d) OPEN (→ folded into the Harbor-creds batch).** ✅ (a) dead `ARGOCD_NAMESPACE` guard gone from `45-install-traefik.sh`. ✅ (b) `90-e2e-istio-existing.sh:257` uses `ensure_namespace "$PROBE_NS"`. ✅ (e) `test-classify-changes.sh` now pins `.claude/hooks/*.py` + `.claude/settings.json` → code=true AND `.claude/agents/*.md` → docs=true (both directions, RED-proven), + the classifier header's stale `tests/` path fixed. ❌ (c) `22-harbor-robot.sh`'s `mkdir` outside `umask 077` — **WON'T-FIX** (adversary-refuted s3): `mkdir -p` no-ops on the pre-existing `secrets/`, so the change is a near-no-op, and `0700` would flirt with the documented cross-uid CA-traversal hazard (`security.md`: public CAs must be world-readable, but a `0700` dir has no `o+x` to traverse). File is 0600; a dirname is not a secret — left documented-minor. 🔴 (d) `scenario-2.md` restates `robot$<name>` (:87, :283) — reference `secrets/harbor-robot.env`, not the literal; **do this with B14** (the same HARBOR_USERNAME doc story). |
